...
`CLOSE_WAIT` 是被动关闭方，在返回`ACK`后，关闭进程释放资源的过程。所以当服务端出现大量`CLOSE_WAIT`时，说明服务端的程序没有调用`close`关闭连接。

一个标准的TCP流程

1. 创建服务端socket，bind绑定端口、listen监听端口
2. 将服务端socket注册到epoll
3. epoll_wait等待连接到来，调用accpet获取已连接的socket
...
- 2 没有实现，代码中没有将`socket`注册到`epoll`中，导致无法感知连接，就无法调用close
- 3 没有实现，没有调用accpet去获取socket
- 4 没有做，获取到socket连接后，却没有放到epoll中
- 6 没有做，服务端没有执行close函数，代码无法抵达到执行close函数的过程

### 如果客户端以及建立连接后，但是客户端突然出现了故障会发生什么？

客户端崩溃，如果不是系统级的崩溃，那么客户端的操作系统会自动发送断开连接的请求。如果是宕机或断电这种系统级的，这时候服务端是无法捕捉到客户端的信息（杳无音讯）。TCP为了确保这种情况不会发送，提出了保活机制。没一段时间会执行一次心跳检查

当开启保活机制后，要考虑以下三种情况：

- 对方正常响应，TCP保活时间重置
- 对端主机重启，此时服务端发送的请求，在对端看来是无效连接，所以会发送一个RST重新连接
- 对端主机宕机，TCP发送消息无回复，在多次探测后无消息后，就会认为TCP连接已死亡

当然，TCP的保活是在传输层的，应用层层面也可自己实现心跳检测。来进行更加灵活的修改

### 如果已经建立了连接，但是服务端的进程崩溃会发生什么？

进程崩溃，内核会回收该进程所有的TCP连接资源，于是内核会主动发送关闭请求，正常完成TCP四次挥手的过程
